package com.github.cschen1205.mas.flocking.flocks;

import com.github.cschen1205.mas.utils.VehicleState;
import com.github.cschen1205.mas.utils.Vec2I;
...
public class Boid {
    ...
    public double[] getSonar(GameWorld maze)
    {
        return maze.getSonar(getX(), getY(), getCurrentBearing());
    }

    public double[] getAVSonar(GameWorld maze)
    {
        ...
        int r = 0;
        while( y-r>=0 && (this == maze.getBoid(x, y - r) || !maze.hasAgent(x, y-r)) ) r++;
        if (r==0) aSonar[0] = 0.0;
        else aSonar[0] = 1.0 / (double)r;

        r=0;
        while (x+r <= maze.getSize()-1 && y-r>=0 && (this == maze.getBoid(x + r, y - r) || !maze.hasAgent(x+r, y-r) ) ) r++;
        if (r==0) aSonar[1] = 0.0;
        else aSonar[1] = 1.0 / (double)r;

        r=0;
        while (x+r <= maze.getSize()-1 && (this == maze.getBoid(x + r, y) || !maze.hasAgent(x+r, y))) r++;
        if (r==0) aSonar[2] = 0.0;
        else aSonar[2] = 1.0 / (double)r;

        r=0;
        while (x+r<=maze.getSize()-1 && y+r<=maze.getSize()-1 && (this == maze.getBoid(x + r, y + r) || !maze.hasAgent(x+r, y+r))) r++;
        if (r==0) aSonar[3] = 0.0;
        else aSonar[3] = 1.0 / (double)r;

        r=0;
        while (y+r<=maze.getSize()-1 && (this == maze.getBoid(x, y + r) || !maze.hasAgent(x, y+r))) r++;
        if (r==0) aSonar[4] = 0.0;
        else aSonar[4] = 1.0 / (double)r;

        r=0;
        while (x-r>=0 && y+r<=maze.getSize()-1 && (this == maze.getBoid(x - r, y + r) || !maze.hasAgent(x-r, y+r))) r++;
        if (r==0) aSonar[5] = 0.0;
        else aSonar[5] = 1.0 / (double)r;

        r=0;
        while (x-r>=0 && (this == maze.getBoid(x - r, y) || !maze.hasAgent(x-r, y))) r++;
        if (r==0) aSonar[6] = 0.0;
        else aSonar[6] = 1.0 / r;

        r=0;
        while (x-r>=0 && y-r>=0 && (this == maze.getBoid(x - r, y - r) || !maze.hasAgent(x-r, y-r))) r++;
        if (r==0) aSonar[7] = 0.0;
    ...
    public int move(GameWorld maze, int d) {
    ...
    public Vec2I virtual_move(GameWorld maze, int d)
    {
